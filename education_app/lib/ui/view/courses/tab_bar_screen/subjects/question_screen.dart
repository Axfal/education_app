...
class _QuestionScreenState extends State<QuestionScreen> {
                    ...
                    Widget buildOption(int optionIndex, String optionText) {
                      return ListTile(
                        ...
                        leading: Radio<int>(
                          ...
                          onChanged: provider.isSubmitted[question.id] ==
                                      true ||
                                  provider
                                      .isQuestionAlreadySubmitted(question.id)
                              ? null
                              : (value) =>
                                  provider.onChangeRadio(question.id, value!),
                        ),
                        trailing:
  ...
  Icon? _buildTrailingIcon(
    ...
    if (!provider.isSubmitted[question.id]! &&
  ...
  //             textStyle: TextStyle(fontSize: 18),
  //           ),
  //           onPressed: provider.isSubmitted[question.id] == true ||
  //                   provider.isQuestionAlreadySubmitted(question.id)
  //               ? null
  ...
  Widget _buildBookmarkButton(BuildContext context, QuestionsProvider provider,
    ...
    final question = provider.filteredQuestions[index];
    final questionId = provider.filteredQuestions[index].id;
    final isBookmarked = bookMarkProvider.isBookmarked(questionId);
    final canBookmark = provider.isSubmitted[question.id]! ||
